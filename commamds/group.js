...
cmd: "demote|dem",
  ...
  try {
    var botAd = await isBotAdmin(m);
    if (!botAd) return await m.send("_*do I look like admin baka*_");
    var user = m.mentionedJid[0] || m.quoted?.sender || text
    if (!user) return await m.send("✘ Reply to or mention an admin")
    if(!await isadminn(m, user)) return await m.send("you are not admin smart baka")
    let jid = parsedJid(user);
...
cmd: "close",
  ...
  try {
    ...
    return await m.send("✓ this group has bean close");
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
cmd: "open",
  ...
  try {
    ...
    return await m.send("✓ Group opened");
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
cmd: "events|gcevent|grpevents",
  ...
  try {
    ...
    const defaultWelcome = `@pp ╭━━━☆ 𝓦 𝓔 𝓛 𝓒 𝓞 𝓜 𝓔 ☆━━━╮
┃ ➤ Hello @user! Welcome to @gname
┃ ➤ Total Members: @count
┃ ➤ May your stay be legendary and full of fun!
┃ ➤ Remember to respect the Royal Rules 👑
┗━━━━━━━━━━━━━━━━━━━━━━━☆`
    
    const defaultGoodbye = `@pp ╭━━━☆ 𝓖 𝓞 𝓞 𝓓 𝓑 𝓨 𝓔 ☆━━━╮
┃ ➤ @user has left @gname
┃ ➤ Total Members Now: @count
┃ ➤ We hope their time here was legendary! 👑
┗━━━━━━━━━━━━━━━━━━━━━━━☆`
...
@count - Member count
@time - Current time
@pp - Include profile picture
@ad - Include external ad reply`)
    }
...
@ad - Include external ad reply`)
    ...
    return await m.send("*_✘ Invalid option. Use 'events' without parameters to see available commands._*")
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
