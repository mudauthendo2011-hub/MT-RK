 ...
 * All rights reserved to the Royal Code.
...
module.exports = {
  ...
  try {
    ...
    if (!botAd) return await m.send("_Bot need admin power baka.._")
    if (!ad) return await m.send("_You're not admin baka.._")
    
    await m.send(m.quoted, {}, "delete")
    return await m.send(m, {}, "delete")
    }
    
    if (!m.isCreator) return await m.send("_Do I know you😑.._")
    await m.send(m.quoted, {}, "delete")
    if (m.fromMe) {
    return await m.send(m, {}, "delete")
    } else return
...
module.exports = {
  ...
  try {
    ...
    return await m.send('_chat room archived_')
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    ...
    return await m.send('_chat room unarchived_')
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    ...
    return await m.send("_profile pic replaced")
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    ...
    return await m.send("_profile pic kicked out of business.._");
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    ...
    await m.send('_Chat room Cleared_')
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    if (m.isGroup && m.quoted?.sender) {
    await m.client.updateBlockStatus(m.quoted?.sender, "user unblock")
    } else {
    await m.client.updateBlockStatus(m.chat, "user unblock")
    }
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
...
module.exports = {
  ...
  try {
    ...
    const blockList = `_*⨺⃝Х all blocked users⨺⃝Х*_\n\n${num.map(n => `彡 +${n.replace('@s.whatsapp.net', '')}`).join('\n')}`;
...
module.exports = {
  ...
  try {
    ...
    await m.reply(`_Profile name changed to ${q}_`);
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    ...
    await m.send('_Bio changed/made');
  } catch (e) {
    console.log("cmd error", e)
    return await m.sendErr(e)
  }
...
module.exports = {
  ...
  try {
    ...
    if (!text) return await m.send(`_*show a number/jid!*_\n_example ${cmd} 27799648540\n_${cmd} 27799648540@s.whatsapp.net_\n\nuse ${prefix}jid to get the jid of a chat`)
    let jidd
...
module.exports = {
  ...
  if (!botAd) return await m.send("_*I'm not you admin Baka..!*_")
  ...
  if (text.includes("-s")) {
    if (!m.quoted.sticker) return await m.send("_reply to the sticker to mute baka_")
    var hash = m.quoted.fileSha256 ? Buffer.from(m.quoted.fileSha256).toString('hex') : null
    if (bl.stk.includes(hash)) return await m.send("_sticker was muted baka_")
    bl.stk.push(hash)
    await storeData("blacklisted", _b)
    return await m.send("_sticker is now muted_")
  }
  ...
  if (await isadminn(m, user)) return await m.send("User is an admin baka")
  if (bl.users.includes(user)) return await m.send("_User is already muted baka_")
  bl.users.push(user)
  await storeData("blacklisted", _b)
...
module.exports = {
  ...
  var botAd = await isBotAdmin(m)
  if (!botAd) return await m.send("_*I'm not admin Baka..*_")

  var _b = await getData("blacklisted") || {}
  if (!_b[m.chat]) return await m.send("_no one muted here baka_")

  let bl = _b[m.chat]

  if (text.includes("-s")) {
    if (!m.quoted.sticker) return await m.send("_reply to the sticker to unmute baka_")
    var hash = m.quoted.fileSha256 ? Buffer.from(m.quoted.fileSha256).toString('hex') : null
